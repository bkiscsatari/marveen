...
describe('AGENT_LOCAL_BASE_URL: the local-agent endpoint is its own key', () => {
  ...
  it('is what the agent launcher exports as ANTHROPIC_BASE_URL -- not OLLAMA_URL', () => {
    const src = readFileSync(join(PROJECT_ROOT, 'src', 'runtime', 'anthropic-compat-env.ts'), 'utf8')
    ...
    // MOVED AGAIN (2026-09-22, agent-agnostic merge): the provider chain now
    // lives in src/runtime/anthropic-compat-env.ts; agent-process.ts only
    // re-exports it. Same anchor, new file.
