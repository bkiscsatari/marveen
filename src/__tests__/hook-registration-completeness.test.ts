...
const EXEMPT: Record<string, string> = {
  ...
  'agent-forgalom-kapu.py':
    'LOCAL to this install (commit 69757b7, not upstream): a CLI/scheduled check of the inter-agent message rate (exit 0 = quiet, exit 1 = over threshold, the PM stops the round), not a per-tool-call settings hook; it is invoked by hand or from a scheduled task, never registered in a settings file by design',
...
const CHECKOUT_ONLY: Record<string, string> = {
  'gmail-draft-send-gate.py':
    'LOCAL to this install (2026-09-06, not upstream): PreToolUse gate on the Gmail MCP gmail_draft tool that denies action=send only; the standing rule in this install\'s CLAUDE.md is that the MAIN agent never sends external mail (the owner sends), and only the main agent carries the Gmail MCP server -- seeding it to every agent would register a matcher for a tool none of them has',
