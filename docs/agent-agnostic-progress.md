...
## Upstream v1.39.0 merge (2026-09-22)

Az upstream (`Szotasz/marveen` main, 787e37e, v1.36.0 → v1.39.0, 305 commit) be lett olvasztva a runtime-réteg fölé: merge-commit `3d651f1` (`merge/upstream-v1.39` → `feat/agent-agnostic` → éles `main`). Remote-ok: `origin` = a saját fork (`bkiscsatari/marveen`), `upstream` = az eredeti szerző (oda soha nem pusholunk).

Kézzel feloldott konfliktusok és döntések:
- `scripts/hooks/egress-gate.mjs`: az upstream változat. Az upstream a nyitott karantén-módot (`quarantine_open`/`quarantine_denylist`, 6abbcdf) a `"quarantine_reader_posture": "denylist"` kapcsolóként implementálta újra; az éles `store/egress-allowlist.json` migrálva erre a kulcsra (backup: `store/backups/egress-allowlist.pre-upstream-v1.39.json`). **Különbség:** az upstream tiltólistája csak SSRF-védelem (localhost, belső nevek, privát/link-local IP-k); a saját változat kiszivárogtató-csatorna listája (paste, webhook-gyűjtő, linkrövidítő, bot-API) és a DNS-feloldásos belső-cím ellenőrzés az upstream kódban NINCS. `agent-scaffold.ts` + `quarantine-reader.md` sablon szintén upstream; `egress-open-quarantine.test.ts` törölve.
- `src/web/agent-process.ts`: `resolveProviderEnv` marad a `src/runtime/anthropic-compat-env.ts`-ben (re-export); az upstream `AGENT_LOCAL_BASE_URL` (ollama-ág) oda átvezetve, az `agent-local-base-url.test.ts` erre horgonyozva. Az upstream umask/runAsUser indítósor megtartja a `MARVEEN_EXTRA_MCP_CONFIG` flaget.
- `.claude/settings.json`: upstream hook-készlet + a két lokális kiegészítés (`gmail-draft-send-gate.py`, `telegram@marveen-marketplace`); a két regisztrációs lint (`hook-registration-completeness`, `project-settings-hook-anchor`) indokolt bejegyzést kapott a lokális hookokra (`agent-forgalom-kapu.py` = CLI-ellenőrzés, nem settings-hook).
- `package.json`: `better-sqlite3 ^13` (Node ≥22, upstream szerint; WSL Node 22.23), a runtime-függőségek és scriptek maradnak; lock `npm install`-lal újragenerálva.

Ellenőrzés a klónban: `tsc` tiszta; vitest 507 fájl — a 3 maradó piros környezeti (a `.fakehome` út „marveen”-t tartalmaz; a dev `.env` `MAIN_AGENT_ID`-je), `.env` félretételével zöld; az upstream CI script-tesztjei mind zöldek (ehhez a WSL-be `sqlite3` + `jq` került apt-tal). Éles: `pre-upstream-v1.39` tag = 059cb55, DB-backup `store/backups/claudeclaw.pre-upstream-v1.39.db`, `npm ci` + build, csak a dashboard újraindítva (19:50), flotta és `icuka-channels` érintetlen.

**Függő (owner-döntés):** (1) az `icuka-channels` (fő-agent) újraindítása — az új hookok (staleness-guard, claude-usage, email-approval-gate, WebFetch egress-gate a fő-agenten, memory-frontmatter-gate, taskstate/clear-replay, telegram_progress) csak session-újraindítás után élnek; (2) az `update.sh` reggeli-timer parkolása (MORNCONS1: `icuka-morning.timer` le, a 07:30-as runner-task viszi a napindítót); (3) push a forkra (`origin`) — a GitHub-token nincs a gépen.

## Phase 8 — Headless session-agentek + dashboard-választó + Jev-routed (2026-09-22)

**Ok:** a runtime-adapterek megvoltak, de a flotta hosszú életű agentjeit (`startAgentProcess`, `isAgentRunning`, `sendPromptToSession`, üzenet-router, kanban, reconciler) csak tmux-session-névvel lehetett hajtani, ezért egy `runtime: minimax-cli` config ellenére is Claude TUI indult. Ez a fázis köti be a sub-agenteket a runtime-rétegre.

- `src/web/headless-agents.ts` (új): session-loop `runtime.run(spec, szöveg, { resume })` alapon; per-agent sorozatos sor, állapot (`idle|busy|blocked|auth`), folytatási token a `store/headless-agents.json`-ban (dashboard-restartot túlél; `fresh` start ejti), kör-napló `store/headless-agents/<agent>.jsonl`, auth/limit-hibánál egyszeri értesítés a fő-agent inboxába. `isHeadlessAgent(name)` = feloldott runtime ≠ `claude-tmux` VAGY `modelRouting: "jev"`; a fő-agent és a remote (ssh) agentek sosem.
- `src/web/agent-process.ts`: `agentRunState`, `sessionExistsOnHost`, `getAgentRunningSince`, `startAgentProcess` (a runtime-semleges scaffold — security profile + CLAUDE.md szekciók — `applyRuntimeNeutralScaffold`-ba emelve, mindkét út hívja), `stopAgentProcess`, `sendPromptToSession`, `isSessionReadyForPrompt`, `capturePane` elágazik a loophoz; új `sessionActivityState(session)` a router stuck-könyveléséhez. Így a message-router, kanban-dispatch, schedule-runner (sawTurn a loop `lastTurnAt`-jából, resubmit-lánc kihagyva), reconciler, restart-lock változatlanul működik.
- Pane-alapú őrök kihagyják a headless agenteket: stuck-input-watcher, reauth-healer, channel-health-monitor, channel-monitor (plugin-fél), telegram-inbox-wake, channel-intake-monitor, auto-restart-runner, context-guard-runner, context-restart-gate-runner (model-fallback-runner a null pane miatt eleve kilép).
- API: `AgentSummary` + `runtime`, `provider`, `modelRouting`, `headless`, `runtimeState`, `hooksMode`, `routedProfile`; `/api/agents/activity` és `/status` a loopból; `PUT /api/agents/<id>` fogadja a `runtime`/`provider`/`modelRouting` mezőket (+ a rég kezelt, de nem listázott `modelProfile`); új `GET /api/agents/<id>/session-log`; `GET /api/models/available` → `subscription` (csak telepített+bejelentkezett CLI: `src/web/runtime-availability.ts`, 60 s cache) és `routed` (profil-térkép + MODEL_ROUTING mód).
- UI (`web/index.html`, `web/app.js`): „Előfizetéses CLI-k (headless session)” és „Jev-routed” optgroup a modell-legördülőben (`id@runtime` értékek, dataset), a mentés a választásból vezeti le a runtime/provider/authMode/modelRouting payloadot; kártyán runtime-chip (+állapot), headless agentnél a Terminál helyett „Napló” gomb (session-log modal); részletfejlécen a runtime.
- Jev: `MODEL_ROUTING=all` önálló mód (`routingApplies`), a `routeForRun` `currentProfile`-lal hiszterézist alkalmaz (`shouldSwitch`), a routed agent a `routine_lowcost` célon indul (claude-tmux cél → claude-headless), kézbesítéskor a loop kérdezi a Jevet és vált; a router/kanban `shadowRoute` nem duplikál routed agentre. `agent-config.json` új mező: `modelRouting: "jev"` (`readAgentModelRouting`/`writeAgentModelRouting`).
- Tesztek: `src/__tests__/headless-agents.test.ts` (életciklus, sor, resume-token restart után, runtime-váltásnál token-ejtés, busy/abort, auth/limit + értesítés, kivétel, routed indulás+váltás+hiszterézis-hívás, `all` nélkül nem kérdez, térkép nélkül nem indul); `agent-put-fields`, `model-router`, `runtime-model-routing-b1` frissítve.

**Nem került be:** `agent-terminal` route explicit 409-e headless agentre (a UI nem kínálja a gombot); a scheduled-task „lost” logika headless agentre csak a loop-jelekre épül; a leállítás nem öli meg a folyó CLI-kört (a runtime időkorlátja zárja).
